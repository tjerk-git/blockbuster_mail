...
var PusherManager = {
  ...
  init: function () {
    ...
    this.pusher = new Pusher("34aeee625e438241557b", {
      ...
      forceTLS: true,
      authEndpoint:
        "https://interactionfigure.nl/nhl/blockbusterauth/pusher_auth.php",
    });
    ...
    document.querySelectorAll(".window-controls span").forEach((control) => {
      control.addEventListener("click", () => {
        ...
        this.errorPopup.querySelector(".error-message").textContent =
    ...
    document
      ...
      .addEventListener("submit", this.sendEmail.bind(this));
  },

  connectToChannel: function () {
    this.presenceChannel = this.pusher.subscribe("presence-" + this.CHANNEL_ID);
    this.presenceChannel.bind(
      "pusher:subscription_succeeded",
      this.onSubscriptionSucceeded.bind(this),
    );
  },
  ...
  onSubscriptionSucceeded: function (_data) {
    ...
    this.presenceChannel.bind(
      "client-messagetochannel",
      this.onMessageFromOtherPlayer.bind(this),
    );
  },
  ...
  createRandomAdPopup: function () {
    ...
    const message =
      this.adMessages[Math.floor(Math.random() * this.adMessages.length)];
    ...
    const dragStart = (e) => {
      if (
        e.target === popup.querySelector(".window-header") ||
        e.target.parentNode === popup.querySelector(".window-header")
      ) {
  ...
  createEmailElement: function (email) {
    ...
    emailElement.addEventListener("click", (event) =>
      this.showEmailDetail(email, event),
    );
    return emailElement;
  },
  ...
  showEmailDetail: function (email, event) {
    ...
    document.getElementById("detail-date").textContent =
      this.formatRelativeTime(email.date);
